evaluate_classification<-function(data,cluster_lookup, groups_or_types, variables){
  dev.new()
  options(warn=-1)
  df<-data.frame(data)
  b<-c()
  clusters<-c()
  if(!(groups_or_types=="types")&!(groups_or_types=="groups")){
  ...
  denominator<-attr(data[,num1],"denominator")
...
nmat<-cbind(nmat,GiniCo)
  ...
  library(ggplot2)
  #dev.new()
  g<-ggplot(data=cheese, aes(x=x, y=y, group=names,colour=names)) + ggtitle("Lorenz curves for specified variables")+ geom_line() 
  #dev.new()
  # + scale_colour_manual(values=c(rep("steelblue",length(evaluation)),"black"))
  ...
  for(i in 1:length(variables)){
    cat("Variable: ",variables[i],"   ","Gini Coefficient: ",gini_vec[i],"\n")
  }
  
  news<-data.frame(cbind(gini_vec,variables))
  news$gini_vec<-as.numeric(as.character(news$gini_vec))
  gg <- ggplot(data=news, aes(x=variables,y=gini_vec,group=variables))+geom_bar(fill="steelblue") + ggtitle("Gini Coefficients for variables specified")
  
  print(gg)
  
  aa<-summary(gini_vec)
  sd<-sd(gini_vec)
  cat("\n","Summary statistics for Gini Coefficients: ","\n")
  print(aa)
  cat("Standard deviation: ","\n")
  print(sd)
  return(list(lorenz=g,gini=gg,variable_data=master_list))
}
